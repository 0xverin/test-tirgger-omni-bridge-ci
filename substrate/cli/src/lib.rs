...
pub async fn handle(command: &SubstrateCommand) {
    ...
    match command {
        SubstrateCommand::SetupDevChainBridge => {
            ...
            let resource_id: [u8; 32] = <[u8; 32]>::from_hex("6dbf3f9d61108d592cb424722ba78a9c2e786a3d1436508c9a02d7e48d70e41e").expect("Failed to decode hex string");
            let asset = AssetInfo {
                fee: 0, 
                asset: None
            };
        ...
        SubstrateCommand::DevTestWithdraw => {
            let recipient_address = Vec::<u8>::from_hex("0e209C5dEdFfE34120679A681a0d7d21A360a97f").expect("Failed to decode string");
            let resource_id: [u8; 32] = <[u8; 32]>::from_hex("6dbf3f9d61108d592cb424722ba78a9c2e786a3d1436508c9a02d7e48d70e41e").expect("Failed to decode hex string");
            let amount: u128 = 10_000_000_000_000_000_000;

            let transfer_assets_call = litentry_rococo::tx()
                .bridge_transfer()
                .transfer_assets(amount, recipient_address, 0, resource_id);
